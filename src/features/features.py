...
def create_features(meter_df, weather_df):
    meter_df = create_time_features(meter_df)
    meter_df = create_weather_features(meter_df, weather_df)
...
def create_season_feature(meter_df):
    ...
    date = meter_df.DateTime.dt.month*100 + meter_df.DateTime.dt.day
    ...
    meter_df['Season'] = pd.cut(date,[0,321,620,922,1220,1300],
...
def create_day_type_feature(meter_df):
    ...
    meter_df["Day_type"] = pd.cut(meter_df["DateTime"].dt.weekday,[-1,4,5, 6],
    ...
    meter_df.loc[meter_df["DateTime"].isin(holiday_list), "Day_type"] = "Holiday"
    meter_df.loc[meter_df["DateTime"].isin(day_before_holiday_list), "Day_type"] = "Day before holiday"
...
def create_time_slot_feature(meter_df):
    ...
    meter_df["Time_slot"] = pd.cut(meter_df["DateTime"].dt.hour,[-1,3, 6, 11, 14, 17, 20, 23],
                       labels=["Midnight", 
                               "Early morning", 
                               "Morning", 
                               "Early afternoon", 
